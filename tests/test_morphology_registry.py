...
def test_hebrew_morphology_registry_loads_triliteral_roots():
    ...
    assert len(registry) == len(load_pack_entries("he_logos_micro_v1"))
    ...
    pointed_or = registry.require("he-morph-009")
    assert pointed_or.surface == "אוֹר"
    assert pointed_or.root == "א-ו-ר"
    assert pointed_or.inflection["pointing"] == "tiberian"
