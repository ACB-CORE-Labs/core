...
def test_prior_pack_lemma_resolution_unchanged() -> None:
    for lemma, expected in (
        ...
        # `cause` is owned by en_core_cognition_v1 (mounted at index 0),
        # NOT en_core_causation_v1 — this is pre-existing first-match-wins
        # resolution that the syntax pack (mounted later) does not change.
        # The contract here is "prior resolution unchanged"; we pin the
        # TRUE current owner.  `consequent` genuinely resolves to causation.
        ("cause", "en_core_cognition_v1"),
